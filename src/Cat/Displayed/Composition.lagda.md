...
Similarly, the composition of two discrete fibrations is again a discrete fibration.
...
```agda
  ...
  discrete-∘ ℰ-disc ℱ-disc = ℰ∘ℱ-disc where 
    ...
    ℰ∘ℱ-disc .cart-lift f (y' , y'') = Iso→is-hlevel 0 eqv (Σ-is-hlevel 0 (ℰ.cart-lift f y') λ (x' , f') → ℱ.cart-lift (∫hom f f') y'')
      where
        eqv : Iso _ _
        eqv .fst ((a , b) , c , d) = (a , c) , b , d
        eqv .snd .is-iso.from ((a , c) , b , d) = (a , b) , c , d
        eqv .snd .is-iso.rinv _ = refl
        eqv .snd .is-iso.linv _ = refl
```
