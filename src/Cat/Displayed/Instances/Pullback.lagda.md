...
```agda
module Cat.Displayed.Instances.Pullback where
```
...
<!--
```agda
  private module Change-of-base = Dm Change-of-base
```
-->
...
module _ 
  {o ℓ o' ℓ'} 
  {A B U : Precategory o ℓ} {U' : Displayed U o' ℓ'} 
  (f : Functor A B) 
  (E : Displayed B o' ℓ')
  (m : Functor U A)
  (h' : Displayed-functor (f F∘ m) U' E) 
  where

  private module Disp[] = Dm Disp[ U' , E ]
  open Dr E
  ...
  Factor : Displayed-functor m U' (Change-of-base f E)
  Factor .F₀' = h' .F₀'
  Factor .F₁' = h' .F₁'
  Factor .F-id' = cast[] $ h' .F-id' ∙[] wrap _
  Factor .F-∘' = cast[] $ h' .F-∘' ∙[] wrap _
