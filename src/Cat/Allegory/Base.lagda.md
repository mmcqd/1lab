...
```agda
Rel ℓ .cat .idr {A} {B} R = ext λ x y → □-rec! (λ { (a , b , w) → 
  subst (λ e → ∣ R e y ∣) (sym (out! b)) w }) , 
...
Rel ℓ .cat .idl R = ext λ x y → 
  □-rec! (λ { (a , b , w) → subst (λ e → ∣ R x e ∣) (out! w) b }) ,
  λ w → inc (y , w , inc refl)

Rel ℓ .cat .assoc T S R = ext λ x y → 
  (□-rec! λ { (a , b , w) → □-rec! (λ { (c , d , x) → inc (c , d , inc (a , x , w)) }) b }) ,
  (□-rec! λ { (a , b , w) → □-rec! (λ { (c , d , x) → inc (c , inc (a , b , d) , x) }) w })

Rel ℓ .≤-thin = hlevel!
Rel ℓ .≤-refl x y w = w
Rel ℓ .≤-trans x y p q z = y p q (x p q z)
Rel ℓ .≤-antisym p q = ext λ x y → p x y , q x y

Rel ℓ ._◆_ f g a b = □-map (λ { (x , y , w) → x , g a x y , f x b w })

-- This is nice:
Rel ℓ .dual R = refl
Rel ℓ .dual-∘ = ext λ x y → 
  (□-map λ { (a , b , c) → a , c , b }) ,
  (□-map λ { (a , b , c) → a , c , b })
